  ...
  try {
    await insertDB('users', req.body, async (error, results) => {
      if (error) {
        ...
        var user_lott_account ={};
        user_lott_account.user_srl = results.insertId;
        user_lott_account.wallet = 'LOTT';
        user_lott_account.token_name = 'LOTT';
        user_lott_account.address = account.address;
